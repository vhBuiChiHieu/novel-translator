...
export const api = {
  ...
  updateSettings: (updates: Record<string, unknown>) => request<ProjectSettings>("/settings", { method: "PATCH", body: JSON.stringify(updates) }),
  providers: () => request<ProviderSettings>("/providers"),
  createProvider: (profile: ProviderProfileInput) => request<ProviderProfile>("/providers", { method: "POST", body: JSON.stringify(profile) }),
  updateProvider: (id: string, updates: Partial<ProviderProfileInput>) => request<ProviderProfile>(`/providers/${encodeURIComponent(id)}`, { method: "PATCH", body: JSON.stringify(updates) }),
  deleteProvider: (id: string) => request<void>(`/providers/${encodeURIComponent(id)}`, { method: "DELETE" }),
  activateProvider: (id: string) => request<ProviderProfile>(`/providers/${encodeURIComponent(id)}/activate`, { method: "POST" }),
  providerCredentialStatus: (id: string) => request<{ configured: boolean }>(`/providers/${encodeURIComponent(id)}/credential/status`),
  saveProviderCredential: (id: string, api_key: string) => request<void>(`/providers/${encodeURIComponent(id)}/credential`, { method: "PUT", body: JSON.stringify({ api_key }) }),
  testProvider: (id: string) => request<Record<string, unknown>>(`/providers/${encodeURIComponent(id)}/test`, { method: "POST" }),
...
export type ModelOption = { id: string; label: string; status: "stable" | "preview" };
export type ProviderProfile = { id: string; provider: string; base_url?: string; model: string; request_timeout_seconds: number; max_retries: number; options: Record<string, unknown>; provider_options: Record<string, unknown>; credential_ref?: string; credential_configured: boolean; active: boolean };
export type ProviderSettings = { config_version: number; active_profile: string; profiles: ProviderProfile[]; model_options: Record<string, ModelOption[]> };
export type ProviderProfileInput = { profile_id?: string; provider: string; base_url?: string; model: string; request_timeout_seconds?: number; max_retries?: number; options?: Record<string, unknown>; provider_options?: Record<string, unknown>; credential_ref?: string };
export type PromptVersion = "translation-v1" | "translation-v2";
export type ProjectSettings = { project_name: string; prompt_version: PromptVersion; [key: string]: unknown };
