...
import { ApiError, api, type Chapter, type ChapterPreview, type ContextItem, type Dashboard, type Job, type PromptVersion } from "./api";
import "./styles.css";
...
function SettingsPage() {
  ...
  const [promptVersion, setPromptVersion] = useState<PromptVersion>("translation-v1");
  const [newProfile, setNewProfile] = useState({ id: "", provider: "gemini", model: "gemini-3.7-flash", base_url: "" });
  const savePromptVersion = useMutation({ mutationFn: (version: PromptVersion) => api.updateSettings({ prompt_version: version }), onSuccess: (updated) => { setPromptVersion(updated.prompt_version); setFeedback({ kind: "success", text: "Version prompt của project đã được cập nhật." }); query.invalidateQueries({ queryKey: ["settings"] }); }, onError: (error: unknown) => setFeedback({ kind: "error", text: formatError(error) }) });
  ...
  const remove = useMutation({ mutationFn: api.deleteProvider, onSuccess: (_result, profileId) => { setFeedback({ kind: "success", text: `Đã xóa profile ${profileId}.` }); query.invalidateQueries({ queryKey: ["providers"] }); }, onError: (error: unknown) => setFeedback({ kind: "error", text: formatError(error) }) });
  ...
  useEffect(() => {
    if (settings.data?.prompt_version) setPromptVersion(settings.data.prompt_version);
  }, [settings.data?.prompt_version]);
  if (providers.isLoading || settings.isLoading) return <Loading />;
