...
function SettingsPage() { const settings = useQuery({ queryKey: ["settings"], queryFn: api.settings }); const keyStatus = useQuery({ queryKey: ["key-status"], queryFn: api.apiKeyStatus }); const [key, setKey] = useState(""); const [feedback, setFeedback] = useState<Feedback | null>(null); const keyMutation = useMutation({ mutationFn: () => api.saveApiKey(key), onSuccess: () => { setKey(""); setFeedback({ kind: "success", text: "API key đã được lưu an toàn." }); keyStatus.refetch(); }, onError: (error: unknown) => setFeedback({ kind: "error", text: formatError(error) }) }); if (settings.isLoading) return <Loading />; if (settings.error) return <ErrorBox error={settings.error} />; const model = settings.data?.model as Record<string, unknown> | undefined; return <><PageIntro eyebrow="CONFIGURATION" title="Settings" description="API key là write-only và chỉ hiển thị trạng thái đã cấu hình." /><div className="grid-two"><div className="panel"><div className="panel-heading"><div><span className="eyebrow">MODEL</span><h3>Translation setup</h3></div></div><dl className="details"><dt>Provider</dt><dd>{String(model?.provider ?? "—")}</dd><dt>Model</dt><dd>{String(model?.name ?? "—")}</dd><dt>Prompt</dt><dd>{String(settings.data?.prompt_version ?? "—")}</dd></dl></div><div className="panel"><div className="panel-heading"><div><span className="eyebrow">CREDENTIAL</span><h3>DeepSeek API key</h3></div><StatusPill status={keyStatus.data?.configured ? "configured" : "not configured"} /></div><p className="muted">Credential được lưu ngoài file cấu hình và không bao giờ hiển thị lại.</p><label htmlFor="api-key">API key</label><div className="form-row"><input id="api-key" type="password" autoComplete="off" value={key} onChange={(e) => setKey(e.target.value)} placeholder="sk-…" /><Button variant="primary" onClick={() => keyMutation.mutate()} disabled={!key.trim() || keyMutation.isPending} loading={keyMutation.isPending} loadingLabel="Đang lưu…">Lưu key</Button></div>{feedback && <FeedbackBox feedback={feedback} />}</div></div></>; }
