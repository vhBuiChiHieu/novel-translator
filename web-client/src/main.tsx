...
function ProjectPicker({ current }: { current?: { open: boolean; project?: { project_name: string }; path?: string; validation_errors?: string[] } }) {
  ...
  const query = useQueryClient();
  const mutation = useMutation({ mutationFn: api.open, onSuccess: () => query.invalidateQueries() });
  return <section className="panel hero-panel"><span className="eyebrow">PROJECT PICKER</span><h1>{current?.open ? current.project?.project_name : "Mở một project local"}</h1><p className="muted">Nhập đường dẫn tuyệt đối tới thư mục chứa <code>novel.yaml</code>. Dữ liệu vẫn nằm hoàn toàn trên máy này.</p>{current?.validation_errors?.map((error) => <p className="error-line" key={error}>{error}</p>)}<label htmlFor="project-path">Project path</label><div className="form-row"><input id="project-path" value={path} onChange={(event) => setPath(event.target.value)} placeholder="C:\\projects\\tien-hiep-demo" /><button className="primary" onClick={() => mutation.mutate(path)} disabled={!path || mutation.isPending}>Mở project</button></div>{mutation.error && <ErrorBox error={mutation.error} />}{current?.open && <p className="success">Project đang mở: {current.project?.project_name}</p>}</section>;
}
