...
function ProjectPicker({ current }: { current?: { open: boolean; project?: { project_name: string }; path?: string; validation_errors?: string[] } }) {
  ...
  const pending = openMutation.isPending || createMutation.isPending;
  return <section className="panel hero-panel"><span className="eyebrow">PROJECT PICKER</span><h1>{current?.open ? current.project?.project_name : "Project local"}</h1><p className="muted">Mở project đã có hoặc tạo một project mới. Dữ liệu vẫn nằm hoàn toàn trên máy này.</p>{current?.validation_errors?.map((error) => <p className="error-line" key={error}>{error}</p>)}<div className="form-row compact" role="tablist" aria-label="Project action"><button className={mode === "open" ? "primary" : "secondary"} onClick={() => setMode("open")} role="tab" aria-selected={mode === "open"}>Mở project</button><button className={mode === "create" ? "primary" : "secondary"} onClick={() => setMode("create")} role="tab" aria-selected={mode === "create"}>Tạo project mới</button></div>{mode === "open" ? <><label htmlFor="project-path">Project path</label><div className="form-row"><input id="project-path" value={path} onChange={(event) => setPath(event.target.value)} placeholder="C:\\projects\\tien-hiep-demo" /><button className="primary" onClick={() => openMutation.mutate(path)} disabled={!path || pending}>Mở project</button></div></> : <><label htmlFor="project-parent-path">Thư mục cha</label><div className="form-row"><input id="project-parent-path" value={parentPath} onChange={(event) => setParentPath(event.target.value)} placeholder="C:\\projects" /></div><label htmlFor="project-name">Tên project</label><div className="form-row"><input id="project-name" value={name} onChange={(event) => setName(event.target.value)} placeholder="tien-hiep-demo" /><button className="primary" onClick={() => createMutation.mutate()} disabled={!parentPath || !name || pending}>Tạo và mở</button></div><p className="muted">Project sẽ được tạo thành <code>{parentPath || "C:\\projects"}\{name || "ten-project"}</code> cùng cấu trúc SQLite, source, translated, exports và logs.</p></>}{mutationError && <ErrorBox error={mutationError} />}{current?.open && <p className="success">Project đang mở: {current.project?.project_name}</p>}</section>;
}
...
function ImportPage() {
  ...
  return <><div className="page-intro"><div><span className="eyebrow">SOURCE / IMPORT</span><h1>Nhập chương</h1><p className="muted">Preview trước, sau đó đưa các file chapter_<em>n</em>.txt vào project.</p></div></div><div className="panel"><label htmlFor="source-path">Source directory</label><div className="form-row"><input id="source-path" value={path} onChange={(event) => setPath(event.target.value)} placeholder="C:\\input\\chapters" /><button className="secondary" onClick={() => previewMutation.mutate(path)} disabled={!path || previewMutation.isPending}>Preview</button><button className="primary" onClick={() => importMutation.mutate(path)} disabled={!preview.length || importMutation.isPending}>Import</button></div>{error !== undefined && error !== null ? <ErrorBox error={error} /> : null}{preview.length > 0 && <div className="table-wrap"><table><thead><tr><th>Chapter</th><th>File</th><th>UTF-8</th><th>Preview</th></tr></thead><tbody>{preview.map((item) => <tr key={item.chapter_number}><td>#{item.chapter_number}</td><td>{item.path}</td><td>{item.valid_utf8 ? "✓" : "✕"}</td><td className="truncate">{item.error ?? item.source_text ?? "—"}</td></tr>)}</tbody></table></div>}</div></>;
}
