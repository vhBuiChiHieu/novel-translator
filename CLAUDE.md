...
- Ollama `/api/chat` + httpx — default local model provider; model output is JSON Schema-constrained Pydantic data.
...
- `infrastructure/model/ollama_provider.py` — the only Ollama/httpx adapter; it must never write to the database.
...
- `tests/unit`, `tests/integration`, `tests/provider` — core logic, project/SQLite flow, and mocked Ollama HTTP tests.
